...
def upgrade() -> None:
    op.execute(
        "alter table platform_users add column if not exists display_name varchar(160)"
    )
    op.execute(
    ...
    op.execute(
        ...
        create table if not exists platform_role_permissions (
            ...
            permission_key varchar(160) not null
                references platform_permissions(key) on delete cascade,
    ...
    op.execute(
        """
        create table if not exists platform_user_tenants (
            user_id uuid not null references platform_users(id) on delete cascade,
    ...
    op.execute(
        "create index if not exists ix_platform_user_tenants_tenant "
        "on platform_user_tenants(tenant_id)"
    )
    op.execute(
        """
    ...
    for key, description, group_name in PERMISSIONS:
        escaped_description = _quote(description)
        op.execute(
            "insert into platform_permissions(key, description, group_name) "
            f"values ('{key}', '{escaped_description}', '{group_name}') "
    ...
    op.execute(
        """
        insert into platform_roles(name, description, is_system)
        values
    ...
    role_map = {
        "Administrador": [
            key for key, _, _ in PERMISSIONS if key not in {"tenants.purge"}
        ],
        "Operações": [
    ...
    op.execute(
        ...
        f"select t.id, c.key, true from tenants t cross join "
        f"unnest(array[{capability_values}]) as c(key) "
        "on conflict do nothing"
    )
    op.execute(
        f"""
        ...
        begin
          ...
          from unnest(array[{capability_values}]) as c(key)
          on conflict do nothing;
          return new;
        end;
        $$
    ...
    op.execute("drop trigger if exists trg_seed_tenant_capabilities on tenants")
    op.execute(
        "create trigger trg_seed_tenant_capabilities "
        "after insert on tenants for each row "
        "execute function scheduler_seed_tenant_capabilities()"
    )


def downgrade() -> None:
    op.execute("drop trigger if exists trg_seed_tenant_capabilities on tenants")
    op.execute("drop function if exists scheduler_seed_tenant_capabilities()")
