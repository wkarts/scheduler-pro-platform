...
def _page(*, platform_access: bool) -> HTMLResponse:
    endpoint = (
...
async def tenant_reset_page() -> HTMLResponse:
    return _page(platform_access=False)


@router.get("/platform/password/reset-page", response_class=HTMLResponse)
async def platform_reset_page() -> HTMLResponse:
    return _page(platform_access=True)
