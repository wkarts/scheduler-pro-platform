...
async def webhook(integration_key: str, request: Request, session: AsyncSession = Depends(get_tenant_session)):
    payload = await request.json()
    provider_event_id = str(payload.get("id") or payload.get("event_id") or request.headers.get("x-provider-event-id") or "")
    ...
    return success({"accepted": True, "integration_key": integration_key, "provider_event_id": provider_event_id})
