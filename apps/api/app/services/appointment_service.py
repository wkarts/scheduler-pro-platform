...
class AppointmentService:
    ...
    async def _has_overlap(
        ...
        *,
            ...
            sql = """
                select exists(
                  ...
                  where professional_id = :professional_id::uuid
                    and id <> :ignore_appointment_id::uuid
        ...
        return bool(await self.session.scalar(text(sql), params))
    ...
    async def list(self, *, day: date | None = None, professional_id: str | None = None, customer_id: str | None = None, status: str | None = None) -> list[dict[str, Any]]:
        clauses = ["1=1"]
        params: dict[str, Any] = {}
        if day:
            clauses.append("starts_at >= :day_start and starts_at < :day_end")
    ...
    async def availability(self, *, day: date, professional_id: str, service_id: str | None = None, slot_minutes: int = 30) -> list[dict[str, Any]]:
        ...
        business_rows = (
            await self.session.execute(
                ...
                {"dow": int(day_start.strftime("%w")), "professional_id": professional_id},
            )
        ).mappings().all()
