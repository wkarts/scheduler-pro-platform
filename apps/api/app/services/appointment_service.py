...
class AppointmentService:
    ...
    async def _require_reference(self, table: str, entity_id: str, code: str) -> None:
        exists = await self.session.scalar(text(f"select exists(select 1 from {table} where id=:id::uuid)"), {"id": entity_id})
        if not exists:
    ...
    async def _is_inside_business_hours(self, professional_id: str, starts_at: datetime, ends_at: datetime) -> bool:
        ...
        result = await self.session.scalar(
            ...
            {"dow": dow, "professional_id": professional_id, "start_time": starts_at.timetz().replace(tzinfo=None).isoformat(), "end_time": ends_at.timetz().replace(tzinfo=None).isoformat()},
        )
        return bool(result)

    async def _is_blocked(self, professional_id: str, starts_at: datetime, ends_at: datetime) -> bool:
        result = await self.session.scalar(
            text(
                """
                select exists(
                  ...
                  where (professional_id is null or professional_id = :professional_id::uuid)
                    and tstzrange(starts_at, ends_at, '[)') && tstzrange(:starts_at, :ends_at, '[)')
                )
                """
            ),
            {"professional_id": professional_id, "starts_at": starts_at, "ends_at": ends_at},
        )
        return bool(result)

    async def _has_overlap(self, professional_id: str, starts_at: datetime, ends_at: datetime, *, ignore_appointment_id: str | None = None) -> bool:
        base = """
            select exists(
              ...
              where professional_id = :professional_id::uuid
                ...
                and tstzrange(starts_at, ends_at, '[)') && tstzrange(:starts_at, :ends_at, '[)')
        """
        params: dict[str, Any] = {"professional_id": professional_id, "starts_at": starts_at, "ends_at": ends_at, "busy_statuses": list(BUSY_STATUSES)}
    ...
    async def _ensure_slot_available(self, professional_id: str, starts_at: datetime, ends_at: datetime, *, ignore_appointment_id: str | None = None) -> None:
        if ends_at <= starts_at:
            raise APIError("APPOINTMENT_INVALID_INTERVAL", "Fim deve ser posterior ao início.", 422)
        if not await self._is_inside_business_hours(professional_id, starts_at, ends_at):
            raise APIError("APPOINTMENT_OUTSIDE_BUSINESS_HOURS", "Horário fora do expediente.", 409)
        ...
        if await self._has_overlap(professional_id, starts_at, ends_at, ignore_appointment_id=ignore_appointment_id):
    ...
    async def create(self, payload: dict[str, Any]) -> Appointment:
        ...
        try:
            async with self.session.begin():
                ...
                await self._ensure_slot_available(str(payload["professional_id"]), payload["starts_at"], payload["ends_at"])
                self.session.add(appointment)
                await self.session.flush()
                await self._add_history(appointment.id, appointment.status, "created")
                await NotificationService(self.session).schedule_for_appointment(str(appointment.id), "appointment_created")
            return appointment
    ...
    async def _add_history(self, appointment_id: str, status: str, reason: str | None = None) -> None:
        await self.session.execute(text("insert into appointment_status_history(appointment_id, status, reason) values(:appointment_id::uuid, :status, :reason)"), {"appointment_id": appointment_id, "status": status, "reason": reason})

    @staticmethod
    def _row(row: RowMapping) -> dict[str, Any]:
        return dict(row)

    async def list_appointments(self, *, day: date | None = None, professional_id: str | None = None, customer_id: str | None = None, status: str | None = None) -> list[dict[str, Any]]:
        clauses = ["1=1"]
        params: dict[str, Any] = {}
        if day:
        ...
        rows = (await self.session.execute(text(f"""
            select a.id::text, a.customer_id::text, a.service_id::text, a.professional_id::text,
    ...
    async def update_status(self, appointment_id: str, status: str, reason: str | None = None) -> dict[str, Any]:
        ...
        current = await self.session.scalar(text("select status from appointments where id=:id::uuid"), {"id": appointment_id})
        if current is None:
            raise APIError("APPOINTMENT_NOT_FOUND", "Agendamento não encontrado.", 404)
    ...
    async def availability(self, *, day: date, professional_id: str, service_id: str | None = None, slot_minutes: int = 30) -> list[dict[str, Any]]:
        duration = slot_minutes
        if service_id:
            service_duration = await self.session.scalar(text("select duration_minutes from services where id=:id::uuid and active='true'"), {"id": service_id})
        ...
        business_rows = (await self.session.execute(text("""
            select opens_at, closes_at from business_hours
            where day_of_week=:dow and is_open=true and (professional_id is null or professional_id=:professional_id::uuid)
            order by professional_id nulls last
        """), {"dow": int(day_start.strftime("%w")), "professional_id": professional_id})).mappings().all()
        windows: list[tuple[datetime, datetime]] = []
        if business_rows:
            for row in business_rows:
                opens = row["opens_at"]
                closes = row["closes_at"]
                windows.append((datetime.combine(day, opens, tzinfo=UTC), datetime.combine(day, closes, tzinfo=UTC)))
        else:
        ...
        for start, end_limit in windows:
            ...
            while cursor + service_delta <= end_limit:
                ...
                slots.append({"starts_at": cursor.isoformat(), "ends_at": end.isoformat(), "available": not blocked and not overlap, "professional_id": professional_id, "service_id": service_id})
                cursor += step
        return slots
