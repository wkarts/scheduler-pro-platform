...
class RealtimeEventService:
    ...
    def _timezone(self) -> ZoneInfo:
        name = str(self.session.info.get("tenant_timezone") or "America/Bahia")
        try:
    ...
    async def emit_appointment(
        ...
        *,
        ...
        starts_at_label = (
            starts_at.astimezone(self._timezone()).strftime("%d/%m/%Y %H:%M")
        ...
        title, message_template = _EVENT_MESSAGES.get(
            ...
            (
                "Agenda atualizada",
                "O agendamento de {customer_name} foi atualizado.",
            ),
        )
...
class WebPushService:
    ...
    async def _vapid_row(self) -> tuple[str, str] | None:
        row = (
            await self.session.execute(
                text(
                    """
                    select public_key, private_key_ref
                    from web_push_vapid_keys
                    where singleton=1
                    limit 1
                    """
                )
        ...
        if row is None:
            return None
        return str(row["public_key"]), str(row["private_key_ref"])

    async def ensure_vapid_keys(self) -> tuple[str, str]:
        current = await self._vapid_row()
        if current is not None:
            public_key, private_ref = current
            return public_key, secret_resolver.resolve(private_ref)

        # A primeira ativação de Push pode acontecer simultaneamente em dois
        # dispositivos. O lock garante uma única identidade VAPID por tenant.
        await self.session.execute(
            text("select pg_advisory_xact_lock(hashtext('scheduler-pro-webpush-vapid'))")
        )
        current = await self._vapid_row()
        if current is not None:
            public_key, private_ref = current
        ...
        await self.session.execute(
            text(
                ...
                insert into web_push_vapid_keys(
                  singleton, public_key, private_key_ref, updated_at
                ) values(1, :public_key, :private_key_ref, now())
            ...
            ),
            {
                ...
                "private_key_ref": seal_secret(private_value),
            },
        )
        await self.session.commit()
            ...
            await self.session.execute(
                text(
                    ...
                    on conflict(endpoint) do update set
                      ...
                      last_error=null,
                      updated_at=now()
                    returning id::text, endpoint, active, device_label,
    ...
    async def dispatch_event(self, event: dict[str, Any]) -> dict[str, int]:
        ...
        url = (
            str(event_payload.get("url") or "/#agenda")
        ...
        for subscription in subscriptions:
            ...
            try:
                ...
                failed += 1
            except Exception as exc:  # noqa: BLE001 - falha remota é persistida
                await self.session.execute(
                    text(
                        """
                        update web_push_subscriptions
