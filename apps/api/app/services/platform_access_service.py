...
class PlatformAccessService:
    ...
    async def list_permissions(self) -> list[dict[str, Any]]:
        rows = (
            await self.session.execute(
                text(
                    """
                    select key, description, group_name
        ...
        for permission in permissions:
            await self.session.execute(
                text(
                    ...
                    values(cast(:id as uuid), :permission)
    ...
    async def update_role(
        ...
        exists = (
            await self.session.execute(
                text(
                    "select is_system from platform_roles where id=cast(:id as uuid)"
                ),
                {"id": role_id},
            )
        ).scalar_one_or_none()
        ...
        await self.session.execute(
            ...
            {"id": role_id, "name": name.strip(), "description": description},
        )
        await self.session.execute(
            text(
                "delete from platform_role_permissions where role_id=cast(:id as uuid)"
            ),
            {"id": role_id},
        )
        for permission in permissions:
    ...
    async def delete_role(self, role_id: str, actor_id: str) -> None:
        row = (
            await self.session.execute(
                text(
                    "select name, is_system from platform_roles where id=cast(:id as uuid)"
                ),
                {"id": role_id},
            )
        ).mappings().first()
        ...
        if row["is_system"]:
            raise APIError(
                "IAM_ROLE_SYSTEM",
                "Perfis de sistema não podem ser excluídos; edite as permissões.",
                409,
            )
        await self.session.execute(
            text("delete from platform_roles where id=cast(:id as uuid)"),
            {"id": role_id},
        )
        await self.audit(
            actor_id,
            "platform.role.delete",
            metadata={"role_id": role_id, "name": row["name"]},
        )
        await self.session.commit()
                ...
                text(
                    ...
                    select
                      ...
                      coalesce(
                        (
                          select jsonb_agg(
                            jsonb_build_object('id', r.id::text, 'name', r.name)
                            order by r.name
                          )
            ...
            user_id = (
                await self.session.execute(
                    text(
                        ...
                        values(
                          lower(:email), :password_hash, :display_name,
                          false, true, :must_change
                        )
                        returning id::text
                        """
                    ),
    ...
    async def set_user_access(
        ...
        *,
        ...
        await self.session.execute(
            text(
                "delete from platform_user_roles where user_id=cast(:id as uuid)"
            ),
            {"id": user_id},
        )
        await self.session.execute(
            text(
                "delete from platform_user_tenants where user_id=cast(:id as uuid)"
            ),
            {"id": user_id},
        )
    ...
    async def delete_user(self, user_id: str, actor_id: str) -> None:
        ...
        if row["is_super_admin"]:
            ...
            if count <= 1:
                raise APIError(
                    "IAM_LAST_SUPER_ADMIN",
                    "O último superadministrador não pode ser excluído.",
                    409,
                )
        await self.session.execute(
            text("delete from platform_users where id=cast(:id as uuid)"),
            {"id": user_id},
        )
        await self.audit(
            actor_id,
            "platform.user.delete",
            metadata={"user_id": user_id, "email": row["email"]},
        )
        await self.session.commit()
        ...
        await self.session.execute(
            text(
                ...
                insert into tenant_capabilities(
                  tenant_id, capability_key, enabled, config, updated_at
                )
                select id, :key, :enabled, cast(:config as jsonb), now()
                from tenants
                where id=cast(:id as uuid)
