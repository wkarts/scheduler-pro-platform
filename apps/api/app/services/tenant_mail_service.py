...
class TenantSmtpConfig:
    ...
    def configured(self) -> bool:
        return bool(self.host and self.from_email and self.password_ref)
...
class TenantMailService:
    ...
    async def status(self) -> dict[str, Any]:
        ...
        return {
            ...
            "configured": bool(row["host"] and row["from_email"] and row["password_ref"]),
    ...
    async def configure(self, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        timeout_seconds = int(payload.get("timeout_seconds", current.get("timeout_seconds") or 15))
        ...
        if use_tls and use_ssl:
            raise APIError("TENANT_SMTP_SECURITY_INVALID", "Escolha STARTTLS ou SSL/TLS, não ambos.", 422)
        if enabled and (not host or not from_email or not password_ref):
            raise APIError(
                "TENANT_SMTP_INCOMPLETE",
                "Para ativar o e-mail informe servidor, remetente e senha.",
                422,
            )
        await self.session.execute(
            text(
    ...
    @staticmethod
    def _send_sync(config: TenantSmtpConfig, to: str, subject: str, body: str) -> None:
        ...
        if config.use_ssl:
            with smtplib.SMTP_SSL(config.host, config.port, timeout=config.timeout_seconds, context=context) as client:
                if config.username:
    ...
    async def send(self, to: str, subject: str, body: str) -> None:
        ...
        if config is None:
            raise APIError("TENANT_SMTP_DISABLED", "SMTP do tenant não está ativo.", 409)
        await asyncio.to_thread(self._send_sync, config, to, subject, body)

    async def send_test(self, recipient: str) -> dict[str, Any]:
        recipient = recipient.strip()
        if not recipient:
            raise APIError("TENANT_SMTP_TEST_RECIPIENT_REQUIRED", "Informe o destinatário do teste.", 422)
        await self.send(
            recipient,
