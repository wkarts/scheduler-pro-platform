...
class BuildManagerService:
    ...
    async def _profile(
        ...
        if payload.profile:
            ...
            if profile is None or str(profile.tenant_id) != payload.tenant:
                raise APIError(
                    ...
                    "Perfil de build não encontrado para o tenant.",
                    404,
                )
            return profile
        ...
        return (
            await self.session.execute(
                select(BuildProfile)
                .where(
                    BuildProfile.tenant_id == payload.tenant,
                    BuildProfile.target == target,
                )
    ...
    async def create_build_request(
        ...
        if tenant is None:
            raise APIError(
                "TENANT_NOT_FOUND",
                "Tenant não encontrado para o build.",
                404,
            )
        ...
        if payload.requested_by and requested_by is None:
        ...
        job = BuildJob(
            ...
            artifact_manifest=self._expected_artifacts(target, str(request.id)),
        )
        ...
        self.session.add(
            BuildLog(
                ...
                sequence=1,
                level="INFO",
                message="Build request persisted.",
                context={
        ...
        try:
            inputs = await self._workflow_inputs(
                tenant,
                profile,
                target,
                correlation_id,
            )
            dispatch = await self.github.dispatch(
                workflow_file,
                ref=payload.source_ref or "main",
                inputs=inputs,
            )
        except APIError as exc:
            waiting = exc.code == "GITHUB_ACTIONS_NOT_CONFIGURED"
            job.status = "WAITING_CONFIGURATION" if waiting else "FAILED"
            ...
            self.session.add(
                BuildLog(
                    build_job_id=job.id,
                    sequence=2,
                    level="WARNING" if waiting else "ERROR",
                    message=exc.message,
    ...
    async def _workflow_inputs(
        ...
        default_web_url = f"https://{tenant.slug}.scheduler.argws.com.br"
        if profile:
            default_web_url = str(profile.api_url).removesuffix("/api/v1")
        web_url = str(config.get("web_url") or default_web_url)

        public_name = tenant.name
        branding = (
            await self.session.execute(
                select(TenantBrandingProfile).where(
                    TenantBrandingProfile.tenant_id == tenant.id
                )
            )
        ).scalar_one_or_none()
    ...
    async def refresh_job(self, job_id: str) -> dict[str, object]:
        ...
        runs = await self.github.latest_runs(
            job.workflow_name,
            branch=job.source_ref or "main",
            limit=20,
        )
        match = next(
            (
                ...
                in str(run.get("display_title") or run.get("name") or "")
            ),
            None,
        )
        if match is None:
        ...
        self.session.add(
            BuildLog(
                ...
                sequence=sequence,
                level="INFO",
                message=f"Workflow status synchronized: {job.status}",
                context={
    ...
    async def _next_log_sequence(self, job_id: str) -> int:
        logs = (
            await self.session.execute(
                select(BuildLog).where(BuildLog.build_job_id == job_id)
            )
        ).scalars().all()
        return len(logs) + 1

    @staticmethod
    def _profile_dict(profile: BuildProfile) -> dict[str, object]:
        return {
            "id": profile.id,
    ...
    @staticmethod
    def _log_dict(log: BuildLog) -> dict[str, object]:
        return {
    ...
    @staticmethod
    def _artifact_dict(artifact: BuildArtifact) -> dict[str, object]:
        return {
            "id": artifact.id,
    ...
    @staticmethod
    def _runner_label(target: str) -> str:
        if target in {"ios", "admin-ios"}:
            return "macos-latest"
        if target in {"desktop", "admin-desktop"}:
            return "matrix"
    ...
    @staticmethod
    def _expected_artifacts(target: str, request_id: str) -> dict[str, object]:
        suffix = request_id[:8]
        if target in {"desktop", "admin-desktop"}:
            return {
                "expected": [
                    f"scheduler-pro-{target}-windows-{suffix}",
                    f"scheduler-pro-{target}-linux-{suffix}",
                    f"scheduler-pro-{target}-macos-{suffix}",
                ]
            }
        if target in {"android", "admin-android"}:
            return {
                "expected": [
                    f"scheduler-pro-{target}-debug-installable-{suffix}.apk"
                ]
            }
        if target in {"ios", "admin-ios"}:
            return {"expected": [f"scheduler-pro-{target}-unsigned-{suffix}.ipa"]}
        return {
            "expected": [
