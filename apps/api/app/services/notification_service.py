...
DEFAULT_TEMPLATES = {
    "appointment_created": "Olá, {{customer_name}}! Recebemos sua solicitação de {{service_name}} com {{professional_name}} para {{starts_at_br}}. Aguarde a confirmação.",
...
class NotificationService:
    ...
    async def _appointment_context(self, appointment_id: str, *, reason: str | None = None) -> dict[str, Any] | None:
        ...
        ends_at = row["ends_at"]
        starts_at_br = starts_at.astimezone(UTC).strftime("%d/%m/%Y %H:%M") if isinstance(starts_at, datetime) else ""
        ends_at_br = ends_at.astimezone(UTC).strftime("%d/%m/%Y %H:%M") if isinstance(ends_at, datetime) else ""
        data = self._row(row)
        data.update(
            {
    ...
    async def _enqueue(self, *, appointment_id: str, template_key: str, channel: str, recipient: str, payload: dict[str, Any], scheduled_at: datetime) -> None:
        await self.session.execute(
            text(
                """
                insert into notification_jobs(
                    appointment_id, channel, recipient, template_key, payload, scheduled_at, status, error
                ) values(
                    :appointment_id::uuid, :channel, :recipient, :template_key,
                    cast(:payload as jsonb), :scheduled_at, 'PENDING', null
                )
                on conflict (appointment_id, channel, template_key) where appointment_id is not null
                do update set
    ...
    async def schedule_for_appointment(self, appointment_id: str, event_name: str, *, reason: str | None = None) -> None:
