...
import { CheckCircle2, Download, Globe2, RefreshCw, Share2, Smartphone } from 'lucide-vue-next'
import {
  getPwaInstallState,
...
const mobileArtifacts = computed(() => catalog.value?.artifacts.filter((item) => /(^|[-_])(android|ios)([-_]|$)/i.test(item.target)) || [])
...
<template>
  <Teleport v-if="visible" to=".tenant-console .main-content">
    <section class="sp-universal-downloads">
      <header>
        <div><span>Distribuição universal</span><h2>Aplicativos Scheduler Pro</h2><p>PWA é a experiência principal. APK e IPA permanecem como extensões mobile. Instância atual: <strong>{{ catalog?.tenant.hostname || currentHostname }}</strong>.</p></div>
      ...
      <article class="sp-pwa-card">
        <div class="sp-download-title"><Globe2 :size="25"/><div><strong>Web App / PWA</strong><span>Experiência principal em qualquer dispositivo moderno</span></div></div>
        <p>Instale a WebApp deste tenant. Ela recebe as evoluções do Scheduler Pro diretamente, mantém o foco PWA-first e não depende de um instalador desktop.</p>
      ...
      <div class="sp-download-columns mobile-only">
        <article>
