...
function isMobileViewport(): boolean { return window.matchMedia('(max-width: 900px)').matches }
function setMobileNav(open: boolean): void { mobileOpen.value = open; document.body.classList.toggle('sp-mobile-nav-open', open && isMobileViewport()) }
function toggleShellMenu(): void { if (isMobileViewport()) setMobileNav(!mobileOpen.value); else collapsed.value = !collapsed.value }
function closeMobileNav(): void { setMobileNav(false) }
function onViewportChange(): void { if (!isMobileViewport()) closeMobileNav() }
...
async function cancelAppointment(item: Appointment): Promise<void> {
  if (!await confirmDialog({title:'Cancelar atendimento',message:`Cancelar o atendimento de ${item.customer_name}?`,danger:true,confirmLabel:'Cancelar atendimento'})) return
  try {
...
async function deleteCustomer(item: Customer): Promise<void> { if (!await confirmDialog({title:'Excluir cliente',message:`Excluir o cliente ${item.name}?`,danger:true,confirmLabel:'Excluir'})) return; try { await api(`/customers/${item.id}`, { method: 'DELETE' }); customers.value = await api('/customers'); showToast('Cliente excluído.') } catch (error) { actionError.value = error instanceof Error ? error.message : 'Falha ao excluir cliente.' } }
...
async function deleteService(item: Service): Promise<void> { if (!await confirmDialog({title:'Excluir serviço',message:`Excluir o serviço ${item.name}?`,danger:true,confirmLabel:'Excluir'})) return; try { await api(`/services/${item.id}`, { method: 'DELETE' }); services.value = await api('/services') } catch (error) { actionError.value = error instanceof Error ? error.message : 'Falha ao excluir serviço.' } }
...
async function deleteProfessional(item: Professional): Promise<void> { if (!await confirmDialog({title:'Excluir profissional',message:`Excluir o profissional ${item.name}?`,danger:true,confirmLabel:'Excluir'})) return; try { await api(`/professionals/${item.id}`, { method: 'DELETE' }); professionals.value = await api('/professionals') } catch (error) { actionError.value = error instanceof Error ? error.message : 'Falha ao excluir profissional.' } }
...
async function saveBusinessHour(): Promise<void> { const opens=businessHourForm.value.opens_at; const closes=businessHourForm.value.closes_at; if(!opens||!closes){actionError.value='Informe os horários de abertura e fechamento.';return} if(closes<=opens){actionError.value='O horário de fechamento precisa ser posterior ao horário de abertura.';return} try { await api(editingId.value ? `/schedule/business-hours/${editingId.value}` : '/schedule/business-hours', { method: editingId.value ? 'PUT' : 'POST', body: JSON.stringify({ ...businessHourForm.value, professional_id: businessHourForm.value.professional_id || null }) }); businessHours.value = await api('/schedule/business-hours'); closeEditor(); showToast('Expediente salvo.') } catch (error) { actionError.value = error instanceof Error ? error.message : 'Falha ao salvar expediente.' } }
async function deleteBusinessHour(item: BusinessHour): Promise<void> { if (!await confirmDialog({title:'Excluir expediente',message:'Excluir esta faixa de expediente?',danger:true,confirmLabel:'Excluir'})) return; try { await api(`/schedule/business-hours/${item.id}`, { method: 'DELETE' }); businessHours.value = await api('/schedule/business-hours') } catch (error) { actionError.value = error instanceof Error ? error.message : 'Falha ao excluir expediente.' } }
function toLocalInput(value:string):string { const date=new Date(value); if(Number.isNaN(date.getTime())) return ''; date.setMinutes(date.getMinutes()-date.getTimezoneOffset()); return date.toISOString().slice(0,16) }
function openBlockedPeriod(item?: BlockedPeriod): void { editorKind.value = 'blocked-period'; editingId.value = item?.id || ''; blockedPeriodForm.value = item ? { professional_id: item.professional_id || '', starts_at: toLocalInput(item.starts_at), ends_at: toLocalInput(item.ends_at), reason: item.reason || '' } : { professional_id: '', starts_at: '', ends_at: '', reason: '' }; showEditor.value = true }
async function saveBlockedPeriod(): Promise<void> { const start=new Date(blockedPeriodForm.value.starts_at); const end=new Date(blockedPeriodForm.value.ends_at); if(Number.isNaN(start.getTime())||Number.isNaN(end.getTime())){actionError.value='Informe início e fim válidos para o bloqueio.';return} if(end<=start){actionError.value='O fim do bloqueio precisa ser posterior ao início.';return} try { const wasEditing=Boolean(editingId.value); const payload={ professional_id: blockedPeriodForm.value.professional_id || null, starts_at: start.toISOString(), ends_at: end.toISOString(), reason: blockedPeriodForm.value.reason || null }; await api(wasEditing ? `/schedule/blocked-periods/${editingId.value}` : '/schedule/blocked-periods', { method: wasEditing ? 'PUT' : 'POST', body: JSON.stringify(payload) }); blockedPeriods.value = await api('/schedule/blocked-periods'); closeEditor(); showToast(wasEditing ? 'Bloqueio atualizado.' : 'Bloqueio cadastrado.') } catch (error) { actionError.value = error instanceof Error ? error.message : 'Falha ao salvar bloqueio.' } }
...
<template>
  ...
  <div v-else class="app-shell tenant-console" :class="{ collapsed, mobileOpen }">
    ...
    <div class="content-shell">
      <header class="topbar">
        <button class="icon-button" aria-label="Abrir menu" :aria-expanded="mobileOpen" @click="toggleShellMenu"><Menu :size="20" /></button>
