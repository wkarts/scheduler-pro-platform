...
onUnmounted(() => { stopRealtimeConnection(); window.removeEventListener('hashchange', onHashChange); window.removeEventListener('beforeinstallprompt', onBeforeInstallPrompt); window.removeEventListener('resize', onViewportChange); closeMobileNav(); if (whatsappPoll !== undefined) window.clearInterval(whatsappPoll) })
</script>
