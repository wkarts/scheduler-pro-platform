...
async function loadLanding():Promise<void>{const result=await request<LandingPayload>('/landing?slug=home');landing.value=result.landing_page;branding.value=result.branding;runtimeContext.value=result.context||await request<RuntimeContext>('/context').catch(()=>({}));applyBranding(result.branding);applyMetadata(result.landing_page);if(runtimeContext.value.features?.public_booking){catalog.value=await request<Catalog>('/booking').catch(()=>null)}}
async function loadBooking():Promise<void>{const result=await request<Catalog>('/booking');catalog.value=result;branding.value=result.branding;runtimeContext.value=result.context||await request<RuntimeContext>('/context').catch(()=>({}));applyBranding(result.branding);const html=result.config.booking_template?.content;if(isHtmlContent(html))applyHtmlMetadata(html.html_document,result.config.title||result.branding.app.public_name||'Agendamento online');else document.title=result.config.title||result.branding.app.public_name||'Agendamento online'}
async function loadLogin():Promise<void>{const result=await request<LoginPayload>('/login');loginPage.value=result.login_page.content;branding.value=result.branding;runtimeContext.value=result.context;applyBranding(result.branding);applyHtmlMetadata(result.login_page.content.html_document,result.branding.app.public_name||'Entrar')}
...
<template>
  <main class="public-site-page" :class="{'html-surface':Boolean(landingHtml||bookingHtml||loginHtml)}" :style="bookingPageStyle">
    ...
    <template v-else-if="landingMode&&landing">
      <HtmlTemplateFrame v-if="landingHtml" :html="landingHtml" mode="landing" :context="runtimeContext"/>
    ...
    <template v-else-if="bookingMode&&catalog">
      <HtmlTemplateFrame v-if="bookingHtml" :html="bookingHtml" mode="booking" :context="runtimeContext"/>
