...
type BookingConfig={
  enabled:boolean;title:string;subtitle:string;success_message:string;custom_html:string;allow_any_professional:boolean;require_name:boolean;require_phone:boolean;
  service_mode:FieldMode;email_mode:FieldMode;phone_mode:FieldMode;duration_mode:FieldMode;professional_mode:FieldMode;
...
type PageContent={version:number;global_styles?:Record<string,unknown>;seo?:Record<string,unknown>;blocks?:PageBlock[]}
...
const bookingPageStyle=computed<CSSProperties>(()=>{
  const globals=catalog.value?.config.booking_template?.content?.global_styles||{}
  return {
    '--booking-page-primary':String(globals.primary||'var(--sp-primary,#2563eb)'),
    '--booking-page-secondary':String(globals.secondary||'#071426'),
    '--booking-page-accent':String(globals.accent||'var(--sp-accent,#7c3aed)'),
    '--booking-page-bg':String(globals.background||'#f5f7fb'),
    '--booking-page-text':String(globals.text||'#0f172a'),
    '--booking-page-radius':`${Number(globals.radius||26)}px`,
  } as CSSProperties
})
...
<template>
  <main class="public-site-page" :style="bookingPageStyle">
    <div v-if="loading" class="public-state"><LoaderCircle :size="36" class="spin"/><strong>Carregando página...</strong></div>
    <template v-else-if="landingMode&&landing"><PublicLandingRenderer :content="landing.content" :services="catalog?.services||[]" :professionals="catalog?.professionals||[]" :template-key="landing.template_key"><template #booking><PublicBookingWidget v-if="catalog" :catalog="catalog"/><div v-else class="booking-unavailable"><CalendarDays :size="30"/><strong>Agenda online indisponível neste momento.</strong><span>Você ainda pode usar os contatos desta página.</span></div></template></PublicLandingRenderer></template>
    <template v-else-if="!landingMode&&catalog">
      <section class="direct-booking-shell" :data-template="catalog.config.booking_template?.key||''">
...
<style scoped>
.public-site-page{min-height:100dvh;background:var(--booking-page-bg,#f5f7fb);color:var(--booking-page-text,#0f172a);font-family:Inter,ui-sans-serif,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif}.public-state{min-height:100dvh;display:grid;place-items:center;align-content:center;gap:12px;padding:24px;text-align:center}.public-state p{max-width:560px;color:#52647c}.public-state a{display:inline-flex;min-height:44px;align-items:center;padding:0 16px;border-radius:12px;background:var(--booking-page-primary,#2563eb);color:#fff;text-decoration:none;font-weight:800}.booking-unavailable{display:grid;place-items:center;gap:6px;padding:28px;border:1px dashed #cbd5e1;border-radius:16px;background:#f8fafc;text-align:center;color:#52647c}.booking-unavailable strong{color:#334155}.direct-booking-shell{width:min(100% - 28px,1080px);margin:0 auto;padding:clamp(14px,3vw,32px) 0 48px}.direct-booking-header{overflow:hidden;margin-bottom:18px;padding:clamp(24px,5vw,50px);border-radius:var(--booking-page-radius,26px);background:radial-gradient(circle at 86% 12%,color-mix(in srgb,var(--booking-page-accent,#d6b379) 30%,transparent),transparent 34%),linear-gradient(145deg,var(--booking-page-secondary,#071426),color-mix(in srgb,var(--booking-page-secondary,#071426) 83%,#fff 17%));color:#fff;box-shadow:0 24px 70px rgba(15,23,42,.17)}.direct-brand{display:flex;align-items:center;gap:12px}.direct-brand img{max-width:180px;max-height:54px;object-fit:contain}.direct-brand>div:last-child strong,.direct-brand>div:last-child small{display:block}.direct-brand small{margin-top:3px;color:#e3e8ef}.direct-mark{width:48px;height:48px;border-radius:14px;display:grid;place-items:center;background:var(--booking-page-primary,#2563eb);font-weight:900}.direct-copy{max-width:680px;padding-top:clamp(30px,6vw,58px)}.direct-copy>span{font-size:.72rem;font-weight:900;text-transform:uppercase;letter-spacing:.12em;color:var(--booking-page-accent,#93c5fd)}.direct-copy h1{margin:10px 0 12px;font-size:clamp(34px,7vw,64px);line-height:1;letter-spacing:-.04em}.direct-copy p{margin:0;color:#e1e7ee;font-size:clamp(15px,2vw,18px);line-height:1.6}.spin{animation:site-spin 1s linear infinite}@keyframes site-spin{to{transform:rotate(360deg)}}@media(max-width:680px){.direct-booking-shell{width:min(100% - 16px,1080px);padding-top:8px}.direct-booking-header{padding:24px 18px;border-radius:calc(var(--booking-page-radius,26px)*.8)}.direct-brand img{max-width:140px}.direct-copy h1{font-size:38px}}
</style>
