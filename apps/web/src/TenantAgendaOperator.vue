...
const customerOptions = computed(() => customers.value.map(p => ({ id:p.id, label:p.name, detail:p.phone || p.email || undefined })))
const serviceOptions = computed(() => services.value.filter(p => p.active).map(p => ({id:p.id,label:p.name})))
const professionalOptions = computed(() => professionals.value.map(p => ({id:p.id,label:p.name})))
function searchEntities(kind: keyof Lookup) {
  return async (query: string, signal: AbortSignal): Promise<EntityOption[]> => {
    const data = await api<Lookup>(`/appointments/smart/lookups?q=${encodeURIComponent(query)}&limit=20`, {signal})
    if (signal.aborted) return []
    const merge = <T extends {id:string}>(old: T[], rows:T[]) => [...old.filter(p => !rows.some(row => row.id === p.id)), ...rows]
    customers.value = merge(customers.value,data.customers); services.value = merge(services.value,data.services); professionals.value = merge(professionals.value,data.professionals)
    return data[kind].map(item => ({id:item.id,label:item.name,detail:'phone' in item ? item.phone || undefined : undefined}))
  }
}
const searchCustomers = searchEntities('customers'), searchServices = searchEntities('services'), searchProfessionals = searchEntities('professionals')
...
function applyDefaults():void{
  ...
  if(!selectedProfessionalId.value)selectedProfessionalId.value=professionals.value.find((item)=>item.name===quick.value.professional_name)?.id||''
}
...
function chooseService():void{const item=services.value.find((row)=>row.id===selectedServiceId.value);if(!item)return;freeService.value=item.name;quick.value.duration_minutes=item.duration_minutes>0?item.duration_minutes:(params.value?.default_duration_minutes||60);quick.value.price=item.price??null}
...
function resetQuick(keepDate=false):void{selectedServiceId.value='';selectedProfessionalId.value='';freeService.value='';const date=keepDate?quick.value.starts_at:'';selectedCustomerId.value='';customerSearch.value='';freeService.value='';quick.value={customer_name:'',customer_phone:'',customer_email:'',duration_minutes:params.value?.default_duration_minutes||60,price:null,professional_name:params.value?.default_professional_name||'Agenda geral',starts_at:date};applyDefaults()}
    ...
    <section class="sp-agenda-operator" role="dialog" aria-modal="true" aria-label="Operador da Agenda">
      ...
      <main v-else>
        <section v-if="tab==='quick'" class="sp-operator-form">
          ...
          <template v-else>
            <label>Cliente<EntityCombobox v-model="selectedCustomerId" :options="customerOptions" :search="searchCustomers" label="Cliente" @select="chooseCustomer" /></label>
          </template>

          <div class="sp-operator-grid">
            <label v-if="showService">Serviço <small v-if="!serviceRequired">opcional/livre</small><EntityCombobox v-model="selectedServiceId" v-model:text="freeService" :options="serviceOptions" :search="searchServices" label="Serviço" placeholder="Busque ou digite um novo serviço" :allow-custom="true" :required="serviceRequired" @select="chooseService" /></label>

            <label v-if="showService">Valor<input v-model.number="quick.price" type="number" min="0" step="0.01" placeholder="Opcional"/></label>
            <label v-if="showProfessional">Profissional / responsável<EntityCombobox v-model="selectedProfessionalId" v-model:text="quick.professional_name" :options="professionalOptions" :search="searchProfessionals" label="Profissional" placeholder="Busque ou digite o responsável" :allow-custom="true" @select="chooseProfessional" /></label>
        ...
        <section v-else-if="tab==='recurring'" class="sp-operator-form">
          ...
          <label>Cliente<EntityCombobox v-model="selectedCustomerId" :options="customerOptions" :search="searchCustomers" label="Cliente" :required="true" @select="chooseCustomer" /></label>
          <div class="sp-operator-grid"><label v-if="showService">Serviço<EntityCombobox v-model="selectedServiceId" v-model:text="freeService" :options="serviceOptions" :search="searchServices" label="Serviço" placeholder="Busque ou digite um novo serviço" :allow-custom="true" :required="serviceRequired" @select="chooseService" /></label><label v-if="showProfessional">Profissional<EntityCombobox v-model="selectedProfessionalId" v-model:text="quick.professional_name" :options="professionalOptions" :search="searchProfessionals" label="Profissional" placeholder="Busque ou digite o responsável" :allow-custom="true" @select="chooseProfessional" /></label><label>Primeiro horário<input v-model="quick.starts_at" type="datetime-local"/></label></div>
        ...
        <section v-else-if="tab==='swap'" class="sp-operator-form"><p class="sp-operator-help">Permuta os horários completos de dois agendamentos e solicita nova confirmação.</p><label>Primeiro agendamento<EntityCombobox v-model="swap.first_id" :options="activeAppointments.map(item => ({id:item.id,label:item.customer_name,detail:formatDate(item.starts_at) + ' · ' + item.professional_name}))" label="Agendamento" :required="true" /></label><label>Segundo agendamento<EntityCombobox v-model="swap.second_id" :options="activeAppointments.filter(row=>row.id!==swap.first_id).map(item => ({id:item.id,label:item.customer_name,detail:formatDate(item.starts_at) + ' · ' + item.professional_name}))" label="Agendamento" :required="true" /></label><button class="sp-operator-primary" :disabled="saving" @click="swapSlots"><Shuffle :size="16"/>Permutar horários</button></section>
