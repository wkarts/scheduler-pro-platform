...
async function createQuick(confirmCustomerUpdate=false):Promise<void>{
  ...
  try{
    ...
    success.value='Agendamento criado. O cliente existente foi reutilizado quando o telefone já estava cadastrado.';await loadData();resetQuick(true)
  }catch(exc){
    if(exc instanceof AgendaApiError&&exc.code==='CUSTOMER_PHONE_MATCH_NAME_DIFFERS'){
      ...
      if(window.confirm(`Este telefone já está cadastrado para “${oldName}”. Deseja atualizar o nome para “${newName}” e usar o mesmo cliente?`)){saving.value=false;await createQuick(true);return}
    }
...
async function cancelItem(item:Appointment):Promise<void>{if(!confirm(`Cancelar o agendamento de ${item.customer_name}?`))return;resetMessages();saving.value=true;try{await api(`/appointments/${item.id}/cancel`,{method:'POST',body:JSON.stringify({reason:'Cancelado pelo gestor no Operador da Agenda'})});success.value='Agendamento cancelado e horário liberado.';await loadData()}catch(exc){error.value=exc instanceof Error?exc.message:'Falha ao cancelar o agendamento.'}finally{saving.value=false}}
async function deleteItem(item:Appointment):Promise<void>{if(!confirm(`Excluir definitivamente o registro de ${item.customer_name}? A auditoria será preservada.`))return;resetMessages();saving.value=true;try{await api(`/appointments/${item.id}/permanent`,{method:'DELETE'});success.value='Agendamento removido com auditoria preservada.';await loadData()}catch(exc){error.value=exc instanceof Error?exc.message:'Falha ao excluir o agendamento.'}finally{saving.value=false}}
