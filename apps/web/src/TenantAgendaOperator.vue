...
function resetQuick(keepDate=false):void{const date=keepDate?quick.value.starts_at:'';selectedCustomerId.value='';selectedServiceId.value='';selectedProfessionalId.value='';customerSearch.value='';freeService.value='';quick.value={customer_name:'',customer_phone:'',customer_email:'',duration_minutes:params.value?.default_duration_minutes||60,price:null,professional_name:params.value?.default_professional_name||'Agenda geral',starts_at:date};applyDefaults()}
...
function startReuse(item:Appointment):void{reuseTarget.value=item;tab.value='quick';customerMode.value='new';quick.value.starts_at=localInput(item.starts_at);selectedServiceId.value=item.service_id||'';freeService.value=item.service_name||'';selectedProfessionalId.value=item.professional_id;quick.value.duration_minutes=item.duration_minutes||params.value?.default_duration_minutes||60;quick.value.price=item.price??null;quick.value.customer_name='';quick.value.customer_phone='';quick.value.customer_email=''}
    ...
    <section class="sp-agenda-operator" role="dialog" aria-modal="true" aria-label="Operador da Agenda">
      ...
      <main v-else>
        <section v-if="tab==='quick'" class="sp-operator-form">
          ...
          <template v-else>
            <label>Cliente<EntityCombobox v-model="selectedCustomerId" v-model:text="customerSearch" :options="customers.map(p=>({id:p.id,label:p.name,detail:p.phone||p.email||undefined}))" label="Cliente" placeholder="Buscar cliente por nome ou contato" @select="chooseCustomer"/></label>
          </template>

          <div class="sp-operator-grid">
            <label v-if="showService">Serviço <small v-if="!serviceRequired">opcional/livre</small><EntityCombobox v-model="selectedServiceId" v-model:text="freeService" :options="services.filter(s=>s.active).map(s=>({id:s.id,label:s.name}))" label="Serviço" placeholder="Busque ou digite um serviço" allow-custom :required="serviceRequired" @select="chooseService"/></label>

            <label v-if="showService">Valor<input v-model.number="quick.price" type="number" min="0" step="0.01" placeholder="Opcional"/></label>
            <label v-if="showProfessional">Profissional / responsável<EntityCombobox v-model="selectedProfessionalId" v-model:text="quick.professional_name" :options="professionals.map(p=>({id:p.id,label:p.name}))" label="Profissional" placeholder="Buscar ou informar responsável" allow-custom @select="chooseProfessional"/></label>
        ...
        <section v-else-if="tab==='recurring'" class="sp-operator-form">
          ...
          <label>Cliente<EntityCombobox v-model="selectedCustomerId" :options="customers.map(p=>({id:p.id,label:p.name,detail:p.phone||undefined}))" label="Cliente" @select="chooseCustomer"/></label>
          <div class="sp-operator-grid"><label v-if="showService">Serviço<EntityCombobox v-model="selectedServiceId" v-model:text="freeService" :options="services.filter(s=>s.active).map(s=>({id:s.id,label:s.name}))" label="Serviço" placeholder="Busque ou digite um serviço" allow-custom :required="serviceRequired" @select="chooseService"/></label><label v-if="showProfessional">Profissional<EntityCombobox v-model="selectedProfessionalId" v-model:text="quick.professional_name" :options="professionals.map(p=>({id:p.id,label:p.name}))" label="Profissional" placeholder="Buscar ou informar responsável" allow-custom @select="chooseProfessional"/></label><label>Primeiro horário<input v-model="quick.starts_at" type="datetime-local"/></label></div>
        ...
        <section v-else-if="tab==='swap'" class="sp-operator-form"><p class="sp-operator-help">Permuta os horários completos de dois agendamentos e solicita nova confirmação.</p><label>Primeiro agendamento<EntityCombobox v-model="swap.first_id" :options="activeAppointments.map(p=>({id:p.id,label:p.customer_name,detail:formatDate(p.starts_at)+' · '+p.professional_name}))" label="Agendamento"/></label><label>Segundo agendamento<EntityCombobox v-model="swap.second_id" :options="activeAppointments.filter(p=>p.id!==swap.first_id).map(p=>({id:p.id,label:p.customer_name,detail:formatDate(p.starts_at)+' · '+p.professional_name}))" label="Agendamento"/></label><button class="sp-operator-primary" :disabled="saving" @click="swapSlots"><Shuffle :size="16"/>Permutar horários</button></section>
