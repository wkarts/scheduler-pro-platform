...
import { SchedulerProAdapter, type ArgwsVisualBuilder } from '@argws/visual-builder'
...
type EditorState={content:Record<string,unknown>|HtmlContent;template_key?:string|null;status?:string}
...
let editor:ArgwsVisualBuilder|null=null
let generation=0
...
async function state():Promise<EditorState>{
  const response=await fetch('/api/v1/landing-pages/home',{cache:'no-store',headers:{accept:'application/json'}})
  const payload=await response.json().catch(()=>({})) as Envelope<EditorState>
...
function mountEditor():void{
  ...
  const element=document.createElement('argws-visual-builder') as ArgwsVisualBuilder
  element.classList.add('sp-visual-builder-shell')
  element.adapter=new SchedulerProAdapter({baseUrl:'/api/v1',slug:'home'})
  element.addEventListener('upb-close',close)
  mountPoint.value.appendChild(element)
  editor=element
  void editor.load().catch(exc=>{error.value=exc instanceof Error?exc.message:'Falha ao carregar o Visual Builder.'})
}
...
<template>
  ...
  <Teleport v-if="active" to="body">
    <div ref="mountPoint" class="sp-visual-builder-mount">
      <section v-if="loading" class="builder-state"><strong>Abrindo ARGWS Visual Builder…</strong><span>Carregando somente os recursos necessários para esta página.</span></section>
      ...
      <section v-else-if="htmlProtected" class="html-protected-editor">
        ...
        <div class="html-protected-info"><Code2 :size="20"/><div><strong>Modo HTML visual protegido</strong><span>Use o pacote de template para atualizar este layout. Para uma página nativa do Visual Builder, aplique explicitamente um modelo editável; o autosave nunca substituirá este HTML.</span></div></div>
