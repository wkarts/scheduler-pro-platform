...
async function render():Promise<void>{if(!renderer)return;renderer.context={servicesHtml:servicesHtml(),professionalsHtml:professionalsHtml(),ensureBooking:true};renderer.document=props.content as PageDocument;await nextTick()}
...
onMounted(()=>{if(!host.value)return;renderer=document.createElement('argws-page-renderer') as ArgwsPageRenderer;renderer.addEventListener('upb-rendered',onRendered);host.value.appendChild(renderer);scheduleRender()})
onUnmounted(()=>{cancelAnimationFrame(renderRaf);renderer?.removeEventListener('upb-rendered',onRendered);renderer?.remove();renderer=null;bookingTarget.value=null})
watch(()=>props.content,scheduleRender)
watch(()=>props.services,scheduleRender)
watch(()=>props.professionals,scheduleRender)
</script>

<template><div ref="host" class="sp-visual-public-renderer"/><Teleport v-if="bookingTarget" :to="bookingTarget"><slot name="booking"/></Teleport></template>
<style scoped>.sp-visual-public-renderer{min-height:100dvh}.sp-visual-public-renderer :deep(argws-page-renderer){display:block}</style>
