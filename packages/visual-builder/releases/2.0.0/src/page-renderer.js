...
export class ArgwsPageRenderer extends HTMLElementBase {
  constructor(){super();this.attachShadow({mode:'open'});this._document=normalizeDocument(null);this._device=null;this._context={};this._runtime={};this._observer=null;this._cleanup=null;this._generation=0;}
  set document(value){this._document=normalizeDocument(value);void this.render();}get document(){return this._document;}
  set context(value){this._context=value||{};void this.render();}get context(){return this._context;}
  set runtime(value){this._runtime=value||{};void this.render();}get runtime(){return this._runtime;}
  set deviceOverride(value){this._device=value||null;void this.render();}
  connectedCallback(){if(this.hasAttribute('document')){try{this._document=normalizeDocument(JSON.parse(this.getAttribute('document')));}catch{}}this._observer=globalThis.ResizeObserver?new ResizeObserver(()=>void this.render()):null;this._observer?.observe(this);void this.render();}
  disconnectedCallback(){this._observer?.disconnect();this._observer=null;this._cleanup?.();this._cleanup=null;this._generation+=1;}
