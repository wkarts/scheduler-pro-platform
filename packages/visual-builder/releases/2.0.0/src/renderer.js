import { deepClone, getBreakpoints, getNode, normalizeDocument } from './model.js';
...
export function toSchedulerProContent(input) {
  const doc = normalizeDocument(input); const blocks = [];
  const visit = id => {
...
export function renderDocument(input, { device='desktop', context={}, responsive=true, locale=null } = {}) {
  const normalized=normalizeDocument(input); const doc=locale?localizeDocument(normalized,locale):normalized; const renderContext={...context}; let html=doc.builder.root_ids.map(id=>renderNode(doc,getNode(doc,id),device,renderContext)).join('');
...
export function exportStandaloneHtml(input, options={}) {
  const {html,css,document:doc}=renderDocument(input,{device:'desktop',context:options.context||{},responsive:true,locale:options.locale||null});
