...
export class SchedulerProProjectAdapter {
  ...
  async _loadLanding(){const state=await this.request(`/landing-pages/${encodeURIComponent(this.landingSlug)}`);this.landing.state=state;const candidate=this._meaningful(state?.content)?state.content:(this._meaningful(state?.published_content)?state.published_content:null);if(candidate){const doc=normalizeDocument(candidate);doc.surface='LANDING';return doc;}return this._defaultSurface('LANDING');}
  async _loadSettingSurface(surface,key){const settings=await this.request('/settings/tenant').catch(()=>this.settings||{preferences:{}});this.settings=settings;const value=settings?.preferences?.[key];if(this._meaningful(value)){const doc=normalizeDocument(value);doc.surface=surface;return doc;}return this._defaultSurface(surface);}
  ...
  async loadProject(){const [landingDoc,settings,context,families]=await Promise.all([this._loadLanding(),this.request('/settings/tenant').catch(()=>({preferences:{},slug:'scheduler-pro',timezone:'America/Bahia'})),this.request('/public/context').catch(()=>({features:{},pages:{},capabilities:{}})),this.request('/landing-pages/template-families').catch(()=>[])]);this.settings=settings;this.runtimeContext=context||{};this.templateFamilies=Array.isArray(families)?families:[];const bookingDoc=await this._loadSettingSurface('BOOKING','booking_page_template_content');const loginDoc=await this._loadSettingSurface('LOGIN','login_page_template_content');this.project=createProject({id:`scheduler-${settings?.slug||'tenant'}`,name:'Scheduler Pro — Páginas públicas',pages:[createProjectPage({id:'landing',title:'Landing Page',slug:'pagina',route:'/pagina',surface:'LANDING',document:landingDoc,source:{provider:'scheduler-pro'}}),createProjectPage({id:'booking',title:'Agenda Pública',slug:'agendar',route:'/agendar',surface:'BOOKING',document:bookingDoc,source:{provider:'scheduler-pro'}}),createProjectPage({id:'login',title:'Login',slug:'login',route:'/login',surface:'LOGIN',document:loginDoc,source:{provider:'scheduler-pro'}})],activePageId:'landing',integrations:{scheduler_pro:{landing_slug:this.landingSlug,context:this.runtimeContext}}});return normalizeProject(this.project);}
  async loadPage(pageId){const id=String(pageId);let doc;if(id==='landing')doc=await this._loadLanding();else if(id==='booking')doc=await this._loadSettingSurface('BOOKING','booking_page_template_content');else if(id==='login')doc=await this._loadSettingSurface('LOGIN','login_page_template_content');else{const page=this.project?.pages.find(row=>row.id===id);doc=normalizeDocument(page?.document||createDocument({title:'Página'}));}const page=this.project?.pages.find(row=>row.id===id);if(page)page.document=doc;return doc;}
