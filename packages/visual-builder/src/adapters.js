...
export class SchedulerProProjectAdapter {
  ...
  async listTemplateFamilies(){return this.request('/landing-pages/template-families');}
  async applyTemplateFamily(key){
    await this.request(`/landing-pages/${encodeURIComponent(this.landingSlug)}/template-families/${encodeURIComponent(key)}`,{method:'POST',body:'{}'});
