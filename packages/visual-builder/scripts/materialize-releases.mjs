...
if(!parts.length)throw new Error(`ARGWS Visual Builder ${VERSION}: release Base64 ausente.`)
const encoded=(await Promise.all(parts.map(name=>readFile(join(encodedRoot,name),'utf8')))).join('').replace(/\s+/g,'')
const tgz=Buffer.from(encoded,'base64')
if(!tgz.length)throw new Error(`ARGWS Visual Builder ${VERSION}: release vazia.`)
...
if(manifest.version!==VERSION)throw new Error(`Release inválida: esperado ${VERSION}, recebido ${manifest.version||'sem versão'}.`)
