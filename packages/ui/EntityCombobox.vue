...
function finalize(): void {
  if (exact.value && query.value.trim()) { choose(exact.value); return }
  if (!props.modelValue) {
...
function keyboard(event: KeyboardEvent): void {
  ...
  if (event.key === 'ArrowDown' || event.key === 'ArrowUp') {
    ...
    if (matches.value.length) cursor.value = (cursor.value + delta + matches.value.length) % matches.value.length
